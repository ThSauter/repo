...
/*!
********************************************************************************
@file       GLCD_SPI_LPC1700.c
@brief      LPC1700 low level Graphic LCD (240x320 pixels) driven
            with SPI functions.
            Erweiterte Version fuer RTX-Betriebssystem 
            
@details    Erweiterung der GLCD-Version aus Keil uVision 4.71
					
@author     N. Normann                                             
@version    1.1
@date       07. Dezember 2013
*******************************************************************************/
...
#include <lpc17xx.h>
#include <RTL.h>       // Erweiterung Normann, RTX-Nutzung fuer Mutex
#include "GLCD.h"
...
/* If processor works on high frequency delay has to be increased, it can be 
   ...
   // Aenderung Normann: Wert auf 0 fuer pclk=25 MHz
#define DELAY_2N    0
...
OS_MUT mutGLCD;  //!< Mutex zur Reservierung des Displays, Erweiterung Normann
...
void GLCD_Init (void) {
  ...
  os_mut_init (&mutGLCD);  // Erweiterung Normann
}
...
void GLCD_SetWindow (unsigned int x, unsigned int y, 
  ...
  os_mut_wait (&mutGLCD, 0xFFFF);  // Erweiterung Normann
  ...
  os_mut_release (&mutGLCD);  // Erweiterung Normann
}
...
void GLCD_DisplayChar (unsigned int ln, unsigned int col, 
  ...
  os_mut_wait (&mutGLCD, 0xFFFF);  // Erweiterung Normann
  if ( (c < 0x20) || (c > 0x8F) ) { return; } // Zeichen nicht implementiert
  c -= 32; // 0x20 bekommt Index 0 in Font-Bitmap
  switch (fi) {
    case 0:  /* Font 6 x 8 */
      ...
      break;
    case 1:  /* Font 16 x 24 */
      GLCD_DrawChar(col * 16, ln * 24, 16, 24, 
                                      (unsigned char *)&Font_16x24_h[c * 24]);
      break;
  }
  os_mut_release (&mutGLCD);  // Erweiterung Normann
}
...
/*******************************************************************************
* Print Character to Terminal  20 Cols, 10 Lines                               *
* Process CR, LF, FF, ignore other Control Characters                          *
* Erweiterung Normann                                                          *
*******************************************************************************/
extern void GLCD_PrintChar ( unsigned char ch ) {
  // Parameter fuer GLCD-Terminal
  static const uint32_t FONT = 1;
  static const uint32_t COLMAX = 19;
  static const uint32_t LINMAX = 9;
  static uint32_t col; // statischer Merker fuer aktuelle Spalte
  static uint32_t line; // statischer Merker fuer aktuelle Zeile

  os_mut_wait (&mutGLCD, 0xFFFF);
  ...
  switch (ch)
  {
    ...
    case 0x7F:  // ignore Delete
      break;
    default:
      if ( ch>=0x20 && ch<=0x8F ) // ignore other Control Chars
      {
        if ( line>LINMAX) { line=0; } // Scroll

        // Display normal characters  
        GLCD_DisplayChar ( line, col++, FONT, ch);
        if ( col>COLMAX) { line++; col=0;}
      }
      break;
  }
  os_mut_release (&mutGLCD);
}    
...
void GLCD_Bargraph (unsigned int x, unsigned int y, 
                    unsigned int w, unsigned int h, unsigned int val) {
  int i,j;
  os_mut_wait (&mutGLCD, 0xFFFF);  // Erweiterung Normann
