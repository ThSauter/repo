...
#ifdef WST_LIVE_ANIMATION
	{
		...
		{    
			if( monitorTimeoutEnableFlag == 1 ){ 
				static WSTMonitorTimeoutEvent monitorTimeoutEvent;
				monitorTimeoutEvent.destination = event->getDestination()->owner;
				monitorTimeoutEvent.currentTick = currentTickCount;
				for(int i = 0; i < WST_MONITOR_MAX_PARALLEL_STATES; i++ )
				{
					monitorTimeoutEvent.currentState[i] = this->activeState[i];
				}
		...
		{
			static WSTMonitorEvent monitorEvent;  
			uint8 *eventArgument = (uint8*) ( event + 1 );

			monitorEvent.source = event->getSource();
			monitorEvent.destination = event->getDestination()->owner;
			
			for(int i = 0; i < WST_MONITOR_MAX_PARALLEL_STATES; i++ )
			{
				monitorEvent.currentState[i] = this->activeState[i];
			}

			monitorEvent.id = (uint16)event->getId();
			monitorEvent.currentTick = currentTickCount;
			
			WSTMonitor_sendEvent( (uint8*) &monitorEvent , event->getSize(), eventArgument );
