...
Mittels des Konfigurationsassistenten \includegraphics[width=0.4cm]{images/package.pdf} \begin{itshape}Manage Run-Time Environment\end{itshape} ist es möglich, Software Komponenten einem Keil Projekt hinzuzufügen. Mit dem Ziel eine Ethernet Kommunikation aufzubauen, wird zunächst die aktuellste Version der MDK Middleware dem Projekt hinzugefügt. Jedoch setzt die Network Component in der Version 7.5.0 den Keil CMSIS:CORE in der Version 5.0.1 voraus. Die daraus resultierende Änderung im Ethernet Projekt gegenüber dem Blinky Projekt ist in \autoref{tab:CmsisCore} dargestellt.

\begin{table}
\footnotesize
\centering
\begin{tabular}{p{0.1cm}p{4.1cm}|p{0.2cm}p{2.1cm}l|p{0.2cm}p{2.1cm}l}
	...
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} CORE & \cellcolor{KeilGreen}\CheckedBox  &  & 4.3.0 & \cellcolor{KeilGreen}\CheckedBox &  & 5.0.1 \\
\end{tabular}
\caption{Heraufsetzen des CMSIS:CORE von 4.3.0 auf 5.0.1.}
\label{tab:CmsisCore}
\end{table}

Die Network Component selbst bietet eine Vielzahl an Software Komponenten, die dem Keil Projekt hinzugefügt werden können. Für den vorliegenden Fall der Ethernet-Kommunikation sind die Komponenten und die benötigten CMSIS Treiber gemäß \autoref{tab:NetworkComponent} zu wählen. Zum Betrieb des Display notwendige Anpassungen sind in \autoref{tab:NetworkComponent} orange gekennzeichnet.

\begin{table}[h!]
\centering
\footnotesize
\begin{tabular}{p{0.01mm}p{4.1cm}p{0.2cm}p{2.2cm}lp{0.2cm}p{2.2cm}l}
	\toprule
	& & \multicolumn{3}{ c }{Blinky} &  \multicolumn{3}{ c }{Ethernet} \\ \cmidrule{3-5}  \cmidrule{6-8}
	& Software Component & Sel. & Variant & Version & Sel. & Variant & Version \\
	\midrule
	\cellcolor{Orange} & \includegraphics[width=0.25cm]{images/package.pdf} Board Support & & \tiny{MCB1700} & & \cellcolor{KeilGreen} & \tiny{MCB1700} & \\
	\cellcolor{Orange} & \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Graphic LCD (API) &  &  & 1.0.0 & \cellcolor{KeilGreen} &  & 1.0.0 \\
	\cellcolor{Orange} & \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} Graphic LCD & \Square  &  & 1.0.0 & \cellcolor{KeilGreen}\CheckedBox &  & 5.0.1 \\
	& \includegraphics[width=0.25cm]{images/package.pdf} CMSIS Driver & & & & \cellcolor{KeilGreen} & & \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Ethernet MAC (API) & & & 2.01 & \cellcolor{KeilGreen} &  & 2.1.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} Ethernet MAC & \Square  &  & 2.9 & \cellcolor{KeilGreen}\CheckedBox &  & 2.9.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Ethernet PHY (API) & & & 2.00 & \cellcolor{KeilGreen} &  & 2.1.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} DP83848C & \Square  &  & 6.1 & \cellcolor{KeilGreen}\CheckedBox &  & 6.1.0 \\
	\cellcolor{Orange} & \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} SPI (API) & & & 2.01 & \cellcolor{KeilGreen} &  & 2.2.0 \\
	\cellcolor{Orange} & \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} SPI & \Square  &  & 2.1 & \Square &  & 2.1.0 \\
	\cellcolor{Orange} & \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} SSP & \Square  &  & 2.5 & \cellcolor{KeilGreen}\CheckedBox &  & 2.7.0 \\
	& \includegraphics[width=0.25cm]{images/package.pdf} Device & & & & \cellcolor{KeilGreen} & & \\
	\cellcolor{Orange} & \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} GPDMA & \Square &  & 1.2 & \cellcolor{KeilGreen}\CheckedBox &  & 1.2.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} GPIO & \Square &  & 1.1 & \cellcolor{KeilGreen}\CheckedBox &  & 1.1.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} PIN & \Square &  & 1.0 & \cellcolor{KeilGreen}\CheckedBox &  & 1.0.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} Startup & \cellcolor{KeilGreen}\CheckedBox &  & 1.0.0 & \cellcolor{KeilGreen}\CheckedBox &  & 1.0.0 \\
	& \includegraphics[width=0.25cm]{images/package.pdf} Network & & \tiny{MDK-Pro} & 7.4.0 & \cellcolor{KeilGreen} & \tiny{MDK-Pro} & 7.4.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} CORE & \Square & \tiny{IPv4/IPv6 Release} & 7.4.0 & \cellcolor{KeilGreen}\CheckedBox & \tiny{IPv4/IPv6 Release} & 7.4.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/item.pdf} Legacy API & \Square &  & 7.4.0 & \Square &  & 7.4.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Interface & & & & \cellcolor{KeilGreen} &  & \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/items.pdf} ETH & 0  &  & 7.4.0 & \cellcolor{KeilGreen} 1 &  & 7.4.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} PPP & \Square &  & 7.4.0 & \Square &  & 7.4.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} SLIP & \Square &  & 7.4.0 & \Square &  & 7.4.0 \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Service & & & & & & \\
	& \hspace{3ex}\includegraphics[width=0.25cm]{images/package.pdf} Socket & & & & \cellcolor{KeilGreen} & & \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} BSD & \Square &  & 7.4.0 & \cellcolor{KeilGreen}\CheckedBox &  & 7.4.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} TCP & \Square &  & 7.4.0 & \cellcolor{KeilGreen}\CheckedBox &  & 7.4.0 \\
	& \hspace{6ex}\includegraphics[width=0.25cm]{images/item.pdf} UDP & \Square &  & 7.4.0 & \cellcolor{KeilGreen}\CheckedBox &  & 7.4.0 \\
	\bottomrule
\end{tabular}
\caption{Benötigte Komponenten der Network Component und deren Abhängigkeiten, sowie die Komponenten zum Betreiben des Displays orange.}
\label{tab:NetworkComponent}
\end{table}
...
Bei der RTX Configuration geht es primär um das Konfigurieren der CMSIS-RTOS Tasks. Dabei wird die Datei RTX\_Conf\_CM.c editiert. Keil bietet den Komfort, die Datei bequem über den integrierten \begin{itshape}Configuration Wizard\end{itshape} zu bearbeiten. Der erste Parameter \begin{itshape}Number of concurrent running user threads\end{itshape} im Abschnitt \begin{itshape}Thread Configuration\end{itshape} gibt die Anzahl der Tasks an, die zur gleichen Zeit laufen. Die Tasks mit der Ursache CMSIS-RTOS in Tabelle \autoref{tab:DefaultThreads} sind bei jeder Applikation standardmäßig aktiv, die das CMSIS-RTOS verwenden.

Das Willert RXF bringt durch sein kleines Onboard-Betriebssystem mit \begin{itshape}WST\_\-Monitor\_receiveTask\end{itshape} einen weiteren Task mit sich. Zudem beansprucht die Network Component bei der Initialisierung die beiden Tasks \begin{itshape}netCore\_Thread\end{itshape} und \begin{itshape}netETH\_Thread\end{itshape}.
...
Zu den bisher aufgezählten Tasks kommen schließlich noch die Tasks hinzu, die durch die Applikation an sich gefordert sind. Gemäß der in rot gekennzeichneten Klassen in Abbildung \autoref{fig:ClassDiagramEthernet} sind das die Tasks RunningIndicator, EthernetReceiver, EthernetTransmitter und Joystick.

Da der Task \begin{itshape}os\_idle\_demon\end{itshape} nicht in die Anzahl der gleichzeitig laufenden Tasks mit eingeht. 

\subsubsection*{RTE Device Configuration}
