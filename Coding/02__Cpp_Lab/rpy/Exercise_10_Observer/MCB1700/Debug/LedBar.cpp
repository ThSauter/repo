...
//## class LedBar
LedBar::LedBar(Poti* poti, WST_TSK* myTask) {
...
void LedBar::update(PotiSubject* potiSubject) {
    //#[ operation update(PotiSubject)
    if (potiSubject == _poti)  
    {
        delay = _poti->getPotiVal();  
    }
    //#]
}
...
Poti* LedBar::get_poti() const {
    return _poti;
}

void LedBar::set_poti(Poti* p__poti) {
    _poti = p__poti;
}
