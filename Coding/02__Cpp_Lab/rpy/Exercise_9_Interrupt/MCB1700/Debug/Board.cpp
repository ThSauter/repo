...
Interrupt* Board::getItsInterrupt() const {
    return (Interrupt*) &itsInterrupt;
}
...
void Board::initRelations() {
    ...
    itsInterrupt._setItsBoard(this);
...
IOxfReactive::TakeEventStatus Board::rootState_processEvent() {
    ...
    // State controlling
    if(rootState_active == controlling)
        {
            res = controlling_processEvent();
        }
    return res;
}

void Board::controlling_entDef() {
    rootState_subState = controlling;
    rootState_active = controlling;
    state_3_entDef();
    state_4_entDef();
}

void Board::controlling_exit() {
...
IOxfReactive::TakeEventStatus Board::controlling_processEvent() {
    IOxfReactive::TakeEventStatus res = eventNotConsumed;
    // State state_3
    if(state_3_processEvent() != eventNotConsumed)
        {
            res = eventConsumed;
            if(!IS_IN(controlling))
                {
                    return res;
                }
        }
...
void Board::state_4_entDef() {
    ...
    //#[ state controlling.state_4.updateDisplay.(Entry) 
...
IOxfReactive::TakeEventStatus Board::state_3_processEvent() {
    ...
    switch (state_3_active) {
        ...
        case pause:
        {
            if(IS_EVENT_TYPE_OF(evExtInterrupt_Default_id))
                {
                    runLedBar_entDef();
                    res = eventConsumed;
                }
...
void Board::runLedBar_exit() {
    ...
    if(runLedBar_subState == notifyLedBar)
        {
            cancel(runLedBar_timeout);
            //#[ state controlling.state_3.runLedBar.notifyLedBar.(Exit) 
...
IOxfReactive::TakeEventStatus Board::runLedBar_handleEvent() {
    ...
    if(IS_EVENT_TYPE_OF(evExtInterrupt_Default_id))
        {
            runLedBar_exit();
            state_3_subState = pause;
            state_3_active = pause;
            res = eventConsumed;
        }
