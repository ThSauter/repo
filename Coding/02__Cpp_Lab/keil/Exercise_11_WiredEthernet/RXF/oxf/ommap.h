...
rank(1), smaller(NULL) {
    ...
    private :
        ...
        inline void _switchNode(Item* other) {
            ...
            Item* myNewLarger;
            Item* hisNewLarger;
            uint32 IShouldBeSmaller=0;
            uint32 HeShouldBeSmaller=0;
            uint32 IShouldBeLarger=0;
            uint32 HeShouldBeLarger=0;
            
            uint32 hisNewRank = rank;
            uint32 myNewRank = rank;
    ...
    public :
        ...
        //## auto_generated
        inline uint32 getRank(void) const {
            //#[ auto_generated
            return rank;
            //#]
        }
        ...
        //## auto_generated
        inline void setRank(uint32 p_rank) {
            //#[ auto_generated
            rank = p_rank;
            //#]
        }
    ...
    inline uint32 find(Concept aConcept) const {
        //#[ operation find(Concept ) const
        uint32 status = 0;
    ...
    // Argument const uint32 anIndex :
    // The index
    //## operation getAt(uint32) const
    inline Concept & getAt(const uint32 anIndex) const {
        //#[ operation getAt(uint32) const
        void* item; 
        
        getFirst(item);
        for (uint32 j=0; j<anIndex; j++) {
        	if (item != NULL) {
        		getNext(item);
        	} else {
        		break;
    ...
    inline uint32 isEmpty(void) const {
        //#[ operation isEmpty() const
        return (root == NULL);
        //#]
    }
    ...
    inline uint32 lookUp(const Key & aKey, Concept & aConcept) const {
        //#[ operation lookUp(const Key & ,Concept & ) const
        uint32 status = 0;
        Item* item = lookUp(aKey);
        if (item != NULL) {
...
private :
    ...
    uint32 count_;		//## attribute count_
